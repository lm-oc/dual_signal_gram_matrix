...
from network.ressl4 import ReSSL        #13、15
import time
import os
...
def train(train_loader, model, optimizer, criterion, epoch, iteration_per_epoch, base_lr):
...
def main():
    ...
    if args.dataset == 'cifar10':
        ...
        dataset = ImageFolderPair("./data/shufa/Train", transform=get_contrastive_augment('shufa'),
                                  weak_aug=get_weak_augment('shufa'), style_aug=get_style_augment('shufa'))
        dataset_valid = ImageFolderPair("./data/shufa/Valid", transform=get_contrastive_augment('shufa'),
                                        weak_aug=get_weak_augment('shufa'), style_aug=get_style_augment('shufa'))
    ...
    checkpoint_path = 'checkpoints/ressl-attention-gram-{}.pth'.format(args.dataset)
    ...
    for epoch in range(start_epoch, epochs):
        train(val_loader, model, optimizer, criterion, epoch, iteration_per_epoch, base_lr)

        validation_loss = validate(val_loader, criterion, model)
